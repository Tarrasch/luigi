...
class Task(object):
    ...
    * :py:meth:`output` - the output :py:class:`Target` that this Task creates.
    ...
    """
    This value can be overriden to set the task_namespace that will be used.
    If it's not specified and you try to read this value anyway, it will return
    garbage.

    For debugging purposes. One might call ``MyTask._task_namespace()``.
    """
    task_namespace = __not_user_specified

    @classmethod
    def _task_namespace(cls):
        # classproperties are not a thing in python. Which is what I ideally would have used
        # for ``task_namespace``. But this is not public
        # interface so we work around it with a little big more logic.
...
def _task_wraps(task_class):
    ...
    return functools.wraps(task_class, updated=[])
